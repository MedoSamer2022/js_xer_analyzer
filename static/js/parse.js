...
const setDataType = (col, val) => {
    ...
    if (col.endsWith('_date') || col.endsWith('_date2')) {
        // XER dates typically like "YYYY-MM-DD HH:MM"
        try {
    ...
    if (col.endsWith('_cost') || col.endsWith('_qty') || col.endsWith('_cnt') || col.endsWith('_hr_cnt')) {
...
// Improved % complete calculation with guards
const calcPercent = task => {
    const denomUnits = (task.target_work_qty || 0) + (task.target_equip_qty || 0);
    const unitsPct = denomUnits > 0
        ? 1 - ((task.act_work_qty || 0) + (task.act_equip_qty || 0)) / denomUnits
        : 0;

    const drtnPct = (task.origDur > 0 && task.remDur < task.origDur)
        ? 1 - (task.remDur / task.origDur)
        : 0;

    const pt = {
        CP_Phys: (task.phys_complete_pct || 0) / 100,
        CP_Drtn: drtnPct,
        CP_Units: unitsPct
    }
    return pt[task.complete_pct_type] || 0;
}
...
const newTask = task => {
    ...
    // Choose dates: act vs early
    const startCandidates = [task.act_start_date, task.early_start_date].filter(Boolean)
    const finishCandidates = [task.act_end_date, task.early_end_date].filter(Boolean)
    task.start = startCandidates[0] || task.early_start_date
    task.finish = finishCandidates[0] || task.early_end_date
            ...
            case '%R':
                ...
                switch (currTable) {
                    ...
                    case 'TASKPRED':
                        ...
                        if (!rel.predTask || !rel.succTask) break; // guard
