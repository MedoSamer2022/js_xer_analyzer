...
let resourceChanges = {
    ...
    revisedCost: {
        ...
        getRows: function() {
            return this.data.map(res => {
                const prev = getPrevRes(res);
                const prevCost = prev ? prev.target_cost : 0;
                return [
    ...
    revisedUnits: {
        ...
        getRows: function() {
            return this.data.map(res => {
                const prev = getPrevRes(res);
                const prevQty = prev ? prev.target_qty : 0;
                return [
...
// NEW: Advanced planning checks
let advChecks = {
    negFloat: {
        id: "adv-negfloat",
        title: "Activities with Negative Total Float",
        align: ['left', 'left', 'left', 'right', 'right'],
        columns: ['Act ID', 'Act Name', 'Status', 'Total\r\nFloat', 'Free\r\nFloat'],
        data: [],
        getRows: function() {
            return this.data.map(t => [
                t.task_code, t.task_name, t.status, formatNumber(t.totalFloat), formatNumber(t.freeFloat)
            ])
        }
    },
    freeFloat: {
        id: "adv-freefloat",
        title: "Activities with Zero Free Float",
        align: ['left', 'left', 'left', 'right', 'right'],
        columns: ['Act ID', 'Act Name', 'Status', 'Total\r\nFloat', 'Free\r\nFloat'],
        data: [],
        getRows: function() {
    ...
    lags: {
        id: "adv-lags",
        title: "Relationships with High Lags or Leads",
        align: ['left', 'left', 'left', 'left', 'center'],
        columns: ['Pred ID', 'Pred Name', 'Succ ID', 'Succ Name', 'Link:Lag'],
        data: [],
        getRows: function() {
            return this.data.map(r => [
                r.predTask.task_code, r.predTask.task_name, r.succTask.task_code, r.succTask.task_name, `${r.link}:${r.lag}`
            ])
        }
    },
    redundant: {
        id: "adv-redundant",
        title: "Redundant Logic Between Same Pair",
        align: ['left', 'left', 'left', 'left', 'center', 'center'],
        columns: ['Pred ID', 'Pred Name', 'Succ ID', 'Succ Name', 'Rel A', 'Rel B'],
        data: [],
        getRows: function() {
            return this.data.map(pair => [
                pair[0].predTask.task_code, pair[0].predTask.task_name,
                pair[0].succTask.task_code, pair[0].succTask.task_name,
                `${pair[0].link}:${pair[0].lag}`, `${pair[1].link}:${pair[1].lag}`
            ])
        }
    },
    constraintConflict: {
        id: "adv-constraint-conflict",
        title: "Mandatory Constraints with Conflicting Logic",
        align: ['left', 'left', 'left', 'left', 'center', 'center'],
        columns: ['Act ID', 'Act Name', 'Status', 'Primary Constraint', 'Pred', 'Succ'],
        data: [],
        getRows: function() {
            return this.data.map(t => [
                t.task_code, t.task_name, t.status, t.primeConstraint,
                t.predecessors.length, t.successors.length
            ])
        }
    },
    criticalIntegrity: {
        id: "adv-critical-integrity",
        title: "Critical Path Integrity Issues",
        align: ['left', 'left', 'left', 'center', 'center', 'left', 'left'],
        columns: ['Act ID', 'Act Name', 'Status', 'Pred', 'Succ', 'Primary', 'Secondary'],
        data: [],
        getRows: function() {
            return this.data.map(t => [
                t.task_code, t.task_name, t.status, t.predecessors.length, t.successors.length, t.primeConstraint || 'None', t.secondConstraint || 'None'
            ])
        }
    }
}
